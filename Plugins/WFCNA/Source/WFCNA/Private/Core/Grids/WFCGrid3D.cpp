...
int32 UWFCGrid3D::CombineRotations(int32 RotationA, int32 RotationB) const
{
...
FVector UWFCGrid3D::GetCenterOffsetToOrigin(FIntVector GridLocation) const
{
	FVector OffsetLocation = FVector(GridLocation.X * -CellSize.X, GridLocation.Y * -CellSize.Y, GridLocation.Z * -CellSize.Z);
	if (Dimensions.X % 2 == 0){OffsetLocation.X -= 0.5*CellSize.X;}
	if (Dimensions.Y % 2 == 0){OffsetLocation.Y -= 0.5*CellSize.Y;}
...
FTransform UWFCGrid3D::GetRotationTransform(int32 Rotation) const
{
	...
	*/
	const float Angle = Rotation * 90.0f;
	return FTransform(FRotator(0.f, Angle, 0.f));
}

TSharedPtr<FAsyncGrid> UWFCGrid3D::CreateAsyncGrid()
{
	return MakeShared<FAsyncGrid3D>(Dimensions);
}
