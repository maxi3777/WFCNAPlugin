...
FVector UWFCGrid2D::GetCenterOffsetToOrigin(FIntVector GridLocation) const
{
	FVector OffsetLocation = FVector(GridLocation.X * -CellSize.X, GridLocation.Y * -CellSize.Y, 0.f);
...
TSharedPtr<FAsyncGrid> UWFCGrid2D::CreateAsyncGrid()
{
	return MakeShared<FAsyncGrid2D>(Dimensions);
}
